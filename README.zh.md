...
dsh web 生态的社区插件索引数据源：`community.json` 是 DSH 市场商店插件目录与 dsh-market.com 插件清单（`manifest/plugins.json`）的唯一来源。条目只收录第三方插件作者的仓库链接与元数据——本仓库从不搬运它们的代码。
...
- 消费方：`scripts/market-build` 从本文件派生出 DSH 市场商店与 dsh-market.com 的插件清单。
- 校验：`node scripts/community-index` 对索引执行契约校验（CI 门禁 `pnpm community:check` 同款）。
- 无设置面：本包不再提供任何设置界面（社区插件卡已被 DSH 市场商店的插件目录取代）；保留 inert cordis 行只是为了让既有 profile 与聚合包继续解析该行，安装后无任何 UI。
...
- 新条目进入市场站与商店需运行 `node scripts/market-build` 并提交生成的 `market/dist`（`market:check` 门禁）。
