...
面向 dsh web GUI 设置页的社区插件市场分区：作为唯一「DSH 市场」分区里的社区插件页签（独立安装时保持一级菜单项）直接展开，以插件市场风格网格展示社区贡献的插件（搜索框、分类筛选胶囊、卡片），并配有自己的启用开关。当安装了配套的「插件管理器」插件（`@linxin666/dsh-client-ui-plugin-manager`）且浏览器为本机访问时，卡片可直接在界面内安装与卸载条目；否则分区退化为只读索引，每张卡片保留一键复制安装命令按钮。

## 功能

- **设置分区**：注册为 DSH 市场 hub（dsh-market.tab）里的社区插件页签；未安装该 hub 时回退为自己的独立一级分区。内容直接展开（无折叠层），自带启用开关，由 community-plugins 设置命名空间持久化。
