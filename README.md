...
Community plugin marketplace section for the dsh web GUI settings page: the Community Plugins tab of the single DSH Market settings section (standalone installs keep a first-level settings entry) that opens directly expanded, lists community-contributed plugins in a marketplace-style grid (search box, category filter pills, cards), and carries its own enable switch. When the sibling Plugin manager plugin (`@linxin666/dsh-client-ui-plugin-manager`) is installed and the browser is local, cards install and uninstall entries directly in the GUI; otherwise the section degrades to a read-only index with a one-click copy-install-command button per card.

## What it does

- **Settings surface**: registers the Community Plugins tab into the DSH Market hub section (dsh-market.tab); the same card falls back to its own first-level settings section when the hub is not installed. The content renders directly expanded (no disclosure fold), with its own enable switch backed by the community-plugins settings namespace.
